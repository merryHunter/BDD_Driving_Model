...
# TODO:
# remove videos with low average speed (or if median of speed is 0)
#
...
tf.app.flags.DEFINE_string('output_directory', '/unreliable/DATASETS/europilot/tfrecords-selected/validation', 'Training data directory')
tf.app.flags.DEFINE_string('info_directory', '/unreliable/DATASETS/carla/train-00-info-carla/', 'info files directory')
...
#    print('before')
    try:
        command = command_dict[fprefix]       #get_command(json_path)
        print(command)
    except:
        return 0, False
    sys.stdout.flush()
    ...
    if FLAGS.low_res:
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/encoded': _bytes_feature(image_list),
            'image/speeds': _float_feature(speeds.ravel().tolist()), # ravel l*2 into list
            'image/command': _int64_feature(int(command)),
        }))
